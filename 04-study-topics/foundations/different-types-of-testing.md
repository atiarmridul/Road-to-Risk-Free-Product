...
> **সহজভাবে / In simple words:** Testing types are different ways to examine a product based on purpose, scope, or technique.
>
> **কেন গুরুত্বপূর্ণ / Why it matters:** Knowing the differences helps you choose the right test for each risk.
...
| Testing Type | Focus | Tester Knowledge | Typical Scope |
|---|---|---|---|
| White Box Testing | Internal code and structure | Strong knowledge of implementation/technology | Mostly unit level, can appear in other levels |
| Black Box Testing | External behavior (input/output) | No need to know internal code | Functional behavior of system/features |
| Unit Testing | Smallest code units | Developer/QA with code-level understanding | Function/class/module level |
| System Testing | Full integrated application behavior | Product-level functional understanding | End-to-end behavior of complete system |
...
> **Combined Example: Smoke vs Retesting vs Sanity vs Regression**

Scenario: Mobile Banking App - Fund Transfer Bug

A bug was reported: "User cannot transfer 10,000 BDT - transaction fails."  
Developer fixes it and sends a new build.

| Step | Testing Type | Goal | What You Test | Outcome/Decision |
|---|---|---|---|---|
| 1 | Smoke Testing (after new build) | Check whether the build is testable at all | App opens, login works, dashboard loads, transfer screen opens | If any fail, reject build immediately |
| 2 | Retesting (bug verification) | Confirm the specific bug is fixed | Only the failed case: transfer `10,000 BDT` | Pass = bug fixed; Fail = send back to development |
| 3 | Sanity Testing (quick impact check) | Ensure the fix works in related scenarios | Transfer `10,000`, `9,999`, `10,001`; transfer to another account; check balance update | Confirms feature stability after the fix |
| 4 | Regression Testing (full system check) | Ensure nothing else broke | Login, fund transfer, transaction history, bill payment, notifications | Can reveal side effects (example: transaction history breaks) |

| Quick Memory Line | Meaning |
|---|---|
| Smoke | "Can I even test this build?" |
| Retest | "Is the exact bug fixed?" |
| Sanity | "Does the fix behave correctly?" |
...
| Testing Type | Primary Goal | Who Validates | Stage |
|---|---|---|---|
| Acceptance Testing | Check if system meets requirements/acceptance criteria | Customer, user, or authorized stakeholders | After system testing, before release |
| User Acceptance Testing (UAT) | Validate business fitness and real-world usability | End users/business users | Final validation before deployment |
...
| Testing Type | Structure | Input Style | Main Objective |
|---|---|---|---|
| Adhoc Testing | Unstructured | Random/manual checks | Find loopholes quickly after formal testing |
| Monkey Testing | Highly random (often automated) | Random/unexpected actions and inputs | Check crash resistance and robustness |
| Exploratory Testing | Unscripted but learning-driven | Tester-led scenario exploration | Discover functional/technical issues and insights |
...
| Testing Type | Input Data | Perspective | Main Purpose |
|---|---|---|---|
| Positive Testing | Valid/expected data | Positive path | Verify the application works as intended |
| Negative Testing | Invalid/unexpected data | Error/failure path | Verify graceful handling and robustness |
...
| Testing Type | Environment | Who Tests | Objective |
|---|---|---|---|
| Alpha Testing | Controlled/internal test environment | Internal QA/development teams | Find and fix issues before external release |
| Beta Testing | Real-world user environment | Selected external users | Collect real usage feedback and uncover missed issues |
...
| Testing Type | Focus | Method | Output |
|---|---|---|---|
| Usability Testing | Ease of use and user experience | Observe representative users doing tasks | Pain points, UX improvement opportunities |
| A/B Testing | Comparative performance of variants | Controlled experiment with random user split | Statistically better variant for a goal |
...
| Testing Type | Load Level | Time Window | Goal |
|---|---|---|---|
| Performance Testing | General load | Short to medium | Measure stability and response time |
| Load Testing | Expected/normal peak load | Typical runtime | Validate behavior at designed user volume |
| Stress Testing | Beyond expected load | Typically short bursts | Find breaking points and recovery behavior |
| Endurance Testing | Sustained load | Long duration | Detect degradation, memory leaks, long-run issues |
...
- [Original detailed Notion page](https://app.notion.com/p/43a6aa6a5c41425eaddbbe76e28e7f8b)
